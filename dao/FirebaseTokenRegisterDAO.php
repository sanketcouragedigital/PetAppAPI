...
class FirebaseTokenRegisterDAO
{
    ...
    public function checkFirebaseToken($FirebaseDetails) {
        try {
            $sql = "SELECT * FROM firebase_tokens WHERE device_id='".$FirebaseDetails->getAndroidId()."'";
    ...
    public function saveFirebaseToken($FirebaseDetails) {
        try {
            $sql = "INSERT INTO firebase_tokens(device_id, token)
    ...
    public function updateFirebaseToken($FirebaseDetails) {
        try {
            $sql="UPDATE firebase_tokens SET token='".$FirebaseDetails->getToken()."'
                    WHERE device_id='".$FirebaseDetails->getAndroidId()."' ";
